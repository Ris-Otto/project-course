...
venueController.post("/event/:eventId/update", updateEvent);
venueController.post("/event/:eventId/cancel", cancelEvent);
...
async function addEvent(c: Context) {
    ...
    const bioRes = await Bio
        .create({description: bio.description})
    ...
    const pricingRes = await Pricing
        .create({...pricing})
        .then(data => data.get({plain: true}))

    const eventRes = await Event
        .create({
            name: name, 
            age: age,
            start:start, 
...
async function updateEvent(c: Context) {
    ...
    const bio = await Bio.findOne({where: { id: event.BioId}});
...
async function updateBio(c: Context) {
    ...
    console.log(venue.BioId)
    var bio = await Bio.findOne({where: { id: venue.BioId}});
    if(bio) {
...
async function uploadMedia(c: Context) {
    const media = Media.create({internal:false, href:"lmao"});
