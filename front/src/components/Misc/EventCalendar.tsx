...
function EventInCalendar({event}: EventInCalendarProps) {
    ...
    return (
        <Row className="event-in-calendar" onClick={() => navigate({
            pathname: `/events`,
...
function CalendarInfo({event, start, end}: {event: Event; start: string; end: string}) {
    return (
...
function CalendarDate({date}: {date: Date}) {
    const [month] = useState(() => date.toLocaleString(undefined, { month: 'short' }));
    const [day] = useState(() => date.toLocaleString(undefined, { day: "2-digit" }));
    return (
